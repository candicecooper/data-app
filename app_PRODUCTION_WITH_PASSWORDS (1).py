...
def verify_login(email: str, password: str) -> Optional[Dict[str, Any]]:
    """Verifies if email and password match in staff database."""
    try:
        if not email or not email.strip():
            return None
        ...
        # Check password
        password_hash = staff_member.get('password_hash')
        if not password_hash:
            # No password set - allow login but force password change
            staff_member['must_change_password'] = True
            return staff_member
